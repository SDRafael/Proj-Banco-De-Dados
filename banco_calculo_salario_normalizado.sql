...
--
-- Dumping events for database 'calculo_salario2'
--
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `calcular_salarios`(IN p_bonus DECIMAL(10,2))
BEGIN
  DECLARE bonus DECIMAL(10,2);
  SET bonus = IFNULL(p_bonus, 0);

  TRUNCATE TABLE pessoa_salario;

  INSERT INTO pessoa_salario (cpf, mes, ano, salario_base, bonus, descontos)
  SELECT
    p.cpf,
    MONTH(CURDATE()) AS mes,
    YEAR(CURDATE()) AS ano,
    CASE 
		WHEN p.ativo = 1 THEN c.salario
	END AS salario_base,
    CASE 
		WHEN p.ativo = 1 THEN c.salario * bonus / 100 
        ELSE 0 
	END AS bonus,
    0 AS descontos
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `inserir_pessoa`(
    IN p_cpf CHAR(11),
...
BEGIN
  INSERT INTO pessoa (
    cpf, nome, cidade, email, cep, logradouro, numero, pais,
    usuario, telefone, data_nascimento, cargo_nome, ativo
  ) VALUES (
    p_cpf, p_nome, p_cidade, p_email, p_cep, p_logradouro, p_numero, p_pais,
    p_usuario, p_telefone, p_data_nascimento, p_cargo_nome, 1
  );
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2025-06-15 23:22:18
